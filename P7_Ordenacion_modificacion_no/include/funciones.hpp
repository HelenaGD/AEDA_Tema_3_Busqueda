...
void Print(std::vector<Clave>* secuencia, int size_secuencia) {
  ...
  for (int i = 0; i < size_secuencia; ++i) {
    std::cout << secuencia->at(i).elemento << " ";
  }
  std::cout << "\n";
}
...
void Insercion (std::vector<Clave>* secuencia, int size_secuencia, bool traza) {
  ...
  for (int i = 1; i < size_secuencia; ++i) {
    int j = i - 1;
    aux = secuencia->at(i);
    while ((j >= 0) &&  strnmp(secuencia->at(j).elemento, aux.elemento) && (j < size_secuencia)) {
      if ( traza) {
        std::cout << "\nComparacion: " << aux << " < " << secuencia->at(j) << " : true";
      }
      memcpy(secuencia->at(j+1).elemento, secuencia->at(j).elemento, strlen(secuencia->at(j).elemento)+1);
    ...
    memcpy(secuencia->at(j+1).elemento, aux.elemento, strlen(aux.elemento)+1);
    //secuencia->at(j+1) = aux;

    if (traza) {
      if (j < size_secuencia) {
        std::cout << "\nComparacion: " << aux.elemento << " < " << secuencia->at(j).elemento << " : false";
      } else {
        std::cout << "\nComparacion: " << aux.elemento << " < " << secuencia->at(j+1).elemento << " : false";
      }
      Print(secuencia, size_secuencia);
    }
  }
